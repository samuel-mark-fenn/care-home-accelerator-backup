...
export default class RoomFinder extends LightningElement {
  ...
  handleFindRooms() {
    ...
    if (!this.residentId) {
      this.showToast(
        "Warning",
        "No Resident linked to this Enquiry. Showing all available rooms.",
        "warning"
      );
      ...
      .then((result) => {
        this.rooms = result.map((wrapper) => {
          return {
            ...
            Base_Weekly_Rate__c_Formatted: new Intl.NumberFormat("en-GB", {
              style: "currency",
              currency: "GBP"
            }).format(wrapper.room.Base_Weekly_Rate__c)
          };
        });
  ...
  handleConfirmBooking() {
    ...
    confirmBooking({
      ...
      .then(() => {
        this.showToast(
          "Success",
          "Booking Confirmed! Quote and Contract created.",
          "success"
        );
        this.isLoading = false;
