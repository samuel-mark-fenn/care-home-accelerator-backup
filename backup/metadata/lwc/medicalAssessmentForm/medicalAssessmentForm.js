...
export default class MedicalAssessmentForm extends LightningElement {
  ...
  @wire(getRecord, {
    recordId: "$recordId",
    fields: [OPPORTUNITY_RESIDENT_FIELD]
  })
  ...
  wiredResidents({ error, data }) {
    if (data) {
      this.residentOptions = data.map((res) => ({
        label: res.Name,
        value: res.Id
      }));
    }
  ...
  handleInputChange(event) {
    ...
    else if (fieldName === "Medical_Needs_Assessment__c")
      this.medicalNeeds = value;
    ...
    else if (fieldName === "Mental_Health_Assessment__c")
      this.mentalHealth = value;
    else if (fieldName === "Personal_Care_Assessment__c")
      this.personalCare = value;
    else if (fieldName === "Social_Needs_Assessment__c")
      this.socialNeeds = value;
    else if (fieldName === "Follow_Up_Required__c")
  ...
  handleSubmit() {
    if (!this.residentId || !this.assessmentTypeId) {
      this.showToast(
        "Error",
        "Please select a Resident and Assessment Type.",
        "error"
      );
      return;
    }
